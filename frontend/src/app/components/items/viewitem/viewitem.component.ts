...
export class ViewitemComponent implements OnInit {
  ...
  @ViewChild('sellOfferModal') public sellOfferModal: ModalDirective;
  ...
  constructor(
    ...
    this.sellOfferModal = this.subscription
  ...
  getItemInfo = () => {
    ...
    this.itemService.listItem({
      ...
      if (result.status == true) {
        ...
        this.attributes = JSON.parse(this.itemInfo.attributes);
        this.levels = JSON.parse(this.itemInfo.levels);
        this.stats = JSON.parse(this.itemInfo.stats);
        if (this.currentUser != null) {
          if (this.itemInfo.unlock_content_url == 'true' || (this.itemInfo.owner.id == this.currentUser.user_id)) {
            this.showMedia = true
          } else {
        ...
        if (this.currentUser != null) {
          ...
          this.is_owner = (this.currentUser.user_id == this.itemInfo.owner.id) ? true : false
          if (this.is_owner) {
            this.getListedPrice()
          }
        }
  ...
  /**
   * This is the function which used to find resale information for user
   */
  getListedPrice = () => {
    this.itemService.resaleInformation({
      item_id: this.itemInfo.id
    }).subscribe(result => {
      if (result.result.length == 0) {
        this.isItemResale = false;
      } else {
        this.isItemResale = true;
        this.resalePrice = result.result[0].price;
      }
  ...
  getMoreCollection = () => {
    this.itemService.moreCollection({
      collection_id: this.itemInfo.collection.id,
      item_id: this.itemInfo.id
    }).subscribe(result => {
      if (result.status == true) {
  ...
  */
  openCollection = () => {
    this.router.navigate(["/collection/detail/" + this.itemInfo.collection.id])
  }
  ...
  viewByCategory = () => {
    ...
    this.router.navigate(['/marketplace'], { queryParams: { category_id: this.itemInfo.category.id } })
  }
  ...
  buyNFT = () => {
    ...
    var finalPrice = 0;
    if (this.itemInfo.author_id == this.currentUser.id || this.itemInfo.author_id == this.currentUser.id) {
      finalPrice = originalPrice - admin_royalties;
      addressArray.push(this.itemInfo.owner.metamask_info.id);
      priceArray.push(ethers.utils.parseEther(finalPrice.toFixed(18).toString())._hex)
    } else {
      var royalties = parseFloat(originalPrice) * (parseFloat(this.itemInfo.collection.royalties) / 100);
      ...
      priceArray.push(ethers.utils.parseEther(royalties.toFixed(18).toString())._hex)
    }
    addressArray.push(config.main_address);
    priceArray.push(ethers.utils.parseEther(admin_royalties.toFixed(18).toString())._hex)

    this.spinner = true;
    let that = this;
    this.web3Service.approveContract(this.itemInfo.collection.contract_address, function (approveResult: any) {
      if (approveResult.status == true) {
        that.web3Service.sendMoneyToOwner(addressArray, priceArray, ethers.utils.parseEther(originalPrice.toFixed(18).toString())._hex, function (result: any) {
          if (result.status == true) {
  ...
  sellOfferAction = () => {
    if (!isNaN(this.priceInput)) {
      if (this.priceInput > 0) {
        ...
        this.itemService.sellOffer({
          item_id: this.itemID,
          price: this.priceInput
        }).subscribe(result => {
          this.sellOfferModal.hide();
          this.spinner = false;
          if (result.status == true) {
            this.initFields()
          }
  ...
  deleteMyOffer = (offer: any) => {
    var index = this.offers.findIndex((offer: any) => {
      return offer.sender.id == this.currentUser.id
    })
    if (index = -1) {
  ...
  cancelOffer = (offer: any) => {
    var index = this.offers.findIndex((offer: any) => {
      return offer.receiver.id == this.currentUser.id
    })
    if (index = -1) {
      this.offers.splice(index, 1)
    }
