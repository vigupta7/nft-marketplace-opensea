...
export class ProfileComponent implements AfterViewInit {
  ...
  constructor(
    ...
    this.userService.notifier.subscribe((result: any) => {
  ...
  /**
   * This is the function which used to initate function
   */
  initFields = () => {
    this.currentUser = this.userService.getUser();
    ...
    var user_id = this.router.url.replace('/' + this.type, '').split("/").pop();
  ...
  updateProfileInfo = () => {
    ...
    this.profile_name = this.userInfo.first_name.length > 0 ? this.userInfo.first_name + ' ' + this.userInfo.last_name : this.userInfo.username
  }
  ...
  copyAddress = () => {
    ...
    this.snackBar.open('Address copied', "", { duration: 2000 });
  }
