...
export class ItemService {
  ...
  sellOffer(params: any): Observable<Common> {
    let url = API.base_url + API.item_offer_resale;
    return this.http.post<Common>(url, params).pipe(map((param: any) => new Common().deserialize(param)));
  }

  resaleInformation(params: any): Observable<Common> {
    let url = API.base_url + API.item_offer_resale_info;
    return this.http.post<Common>(url, params).pipe(map((param: any) => new Common().deserialize(param)));
  }
  ...
  getSettingsOption(params: any): Observable<Common> {
    var url = API.base_url + API.settings_option + '?';
    if (params.name) {
